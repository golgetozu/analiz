...
# TAB 1: ÖZET (önceki kod aynen kalacak)
with tab1:
...
with tab5:
    if df_uretim is not None:
        ...
        with col2:
            target_col = st.selectbox("Hedef Değişken", 
                                     ['P Net Prim', 'P Brüt Prim'])
        ...
        if st.button("🚀 GLM Modelini Çalıştır", type="primary"):
            ...
            styled_coef = coef_df.style.applymap(
                highlight_significant, subset=['P-değeri']
            )
            st.dataframe(styled_coef)
            ...
            importance_df = pd.DataFrame({
                'Değişken': coef_df['Değişken'][1:],  # Intercept'i çıkar
...
with tab6:
    if df_uretim is not None:
        ...
        if aktueryal_tip == "Pure Premium Hesaplama":
            ...
            def risk_color(val):
                if val < 80:
                    return 'background-color: #90EE90'  # Düşük risk
                elif val < 120:
                    return 'background-color: #FFFF00'  # Orta risk
                else:
                    return 'background-color: #FF6B6B'  # Yüksek risk
            
            styled_pp = pure_premium.style.applymap(
                risk_color, subset=['Risk Skoru']
            )
            
            st.dataframe(styled_pp)
            ...
            df_uretim['Toplam_Risk'] = (
                ...
                (df_uretim['P Net Prim'] / df_uretim['P Net Prim'].mean())
            )
            ...
            with col1:
                fig = px.pie(values=risk_dagilim.values, 
                           ...
                           color_discrete_map={'Düşük Risk': '#90EE90',
                                              'Orta Risk': '#FFFF00',
                                              'Yüksek Risk': '#FF6B6B'})
                st.plotly_chart(fig, use_container_width=True)
            ...
            with col2:
                ...
                fig2 = px.bar(x=risk_prim.index, y=risk_prim.values,
                            ...
                            color=risk_prim.index,
                            color_discrete_map={'Düşük Risk': '#90EE90',
                                               'Orta Risk': '#FFFF00',
                                               'Yüksek Risk': '#FF6B6B'})
                st.plotly_chart(fig2, use_container_width=True)

# TAB 7: RİSK SKORLAMA (YENİ!)
with tab7:
    if df_uretim is not None:
        ...
        if st.button("📊 Risk Skorlarını Hesapla", type="primary"):
            ...
            # Model yılı risk skoru
            df_uretim['model_yili_skor'] = (2024 - df_uretim['MODEL YILI']) / 10 * model_yili_agirlik
            
            # İl risk skoru
            df_uretim['il_skor'] = df_uretim['U Sig. İl'].map(il_risk_norm) * il_agirlik
            
            # Marka risk skoru
            df_uretim['marka_skor'] = df_uretim['MARKA'].map(marka_risk_norm) * marka_agirlik
            ...
            riskli_segmentler = df_uretim.nlargest(10, 'risk_skoru')[
                ['Poliçe No', 'U Sig. İl', 'MARKA', 'MODEL YILI', 'P Net Prim', 'risk_skoru']
            ]
            
            st.dataframe(riskli_segmentler)
            
            # Risk bazlı fiyat önerisi
